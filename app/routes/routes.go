...
func InitHTTPRoute(g *gin.Engine, database dbresolver.DB, redisdb redisdb.RedisInterface, mongodbClient mongodb.MongoDBInterface, opensearchClient opensearch_dbo.OpenSearchClientInterface, kafkaClient kafkadbo.KafkaClientInterface, ctx context.Context) {
	...
	basicAuthRootGroup.Use()
	{
		...
		uploadControllerGroup.Use()
		{
			...
			uploadControllerGroup.GET(constants.SALES_ORDERS_PATH+"/"+constants.UPLOAD_SO_PATH+"/retry/:so-upload-history-id", uploadController.RetryUploadSO)
			uploadControllerGroup.GET(constants.DELIVERY_ORDERS_PATH+"/"+constants.UPLOAD_DO_PATH+"/retry/:sj-upload-history-id", uploadController.RetryUploadDO)
			uploadControllerGroup.GET(constants.SOSJ_PATH+"/"+constants.UPLOAD_SOSJ_PATH+"/retry/:sosj-upload-history-id", uploadController.RetryUploadSOSJ)

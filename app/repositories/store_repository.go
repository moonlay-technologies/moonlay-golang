...
func (r *store) GetIdByStoreCode(storeCode string, countOnly bool, ctx context.Context, resultChan chan *models.StoreChan) {
	...
	if err == redis.Nil {
		err = r.db.QueryRow("SELECT COUNT(*) as total FROM stores WHERE stores.deleted_at IS NULL AND IF((SELECT COUNT(store_code) FROM stores WHERE stores.store_code = ?), stores.store_code = ?, stores.alias_code = ?)", storeCode, storeCode, storeCode).Scan(&total)

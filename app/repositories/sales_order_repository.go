...
type salesOrder struct {
	salesOrderJourneysRepository repositories.SalesOrderJourneysRepositoryInterface
	db                           dbresolver.DB
	redisdb                      redisdb.RedisInterface
}

func InitSalesOrderRepository(salesOrderJourneysRepository repositories.SalesOrderJourneysRepositoryInterface, db dbresolver.DB, redisdb redisdb.RedisInterface) SalesOrderRepositoryInterface {
	return &salesOrder{
...
func (r *salesOrder) UpdateByID(id int, request *models.SalesOrder, isInsertToJourney bool, reason string, sqlTransaction *sql.Tx, ctx context.Context, resultChan chan *models.SalesOrderChan) {
	...
	if request.EndCreatedDate != nil {
		query := fmt.Sprintf("%s='%v'", "end_created_date", request.EndCreatedDate.Format(constants.DATE_TIME_FORMAT_COMON))
		rawSqlQueries = append(rawSqlQueries, query)
	}

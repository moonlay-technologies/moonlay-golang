...
func (r *product) GetByID(ID int, countOnly bool, ctx context.Context, resultChan chan *models.ProductChan) {
	...
	if err == redis.Nil {
		...
		if countOnly == false {
			...
			err = r.db.QueryRow(""+
				"SELECT id, SKU, aliasSKU, productName, description, category_id, unitMeasurementSmall, unitMeasurementMedium, unitMeasurementBig,  isActive, nettWeight  from products as p "+
				"WHERE p.deleted_at IS NULL AND p.id = ?", ID).
				Scan(&product.ID, &product.Sku, &product.AliasSku, &product.ProductName, &product.Description, &product.CategoryID, &product.UnitMeasurementSmall, &product.UnitMeasurementMedium, &product.UnitMeasurementBig, &product.IsActive, &product.NettWeight)

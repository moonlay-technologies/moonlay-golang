...
type WarehouseRepositoryInterface interface {
	...
	GetByCode(code string, countOnly bool, ctx context.Context, resultChan chan *models.WarehouseChan)
	GetByAgentID(agentID int, countOnly bool, ctx context.Context, resultChan chan *models.WarehouseChan)
}
...
func (r *warehouse) GetByCode(code string, countOnly bool, ctx context.Context, resultChan chan *models.WarehouseChan) {
	response := &models.WarehouseChan{}
	var warehouse models.Warehouse
	var total int64

	warehouseRedisKey := fmt.Sprintf("%s:%s", constants.WAREHOUSE_BY_CODE, code)
	warehouseOnRedis, err := r.redisdb.Client().Get(ctx, warehouseRedisKey).Result()

	if err == redis.Nil {
		err = r.db.QueryRow("SELECT COUNT(*) as total FROM warehouses as w WHERE w.code = ?", code).Scan(&total)
...
func (r *warehouse) GetByAgentID(agentID int, countOnly bool, ctx context.Context, resultChan chan *models.WarehouseChan) {
	response := &models.WarehouseChan{}
	var warehouse models.Warehouse
	var total int64

	warehouseRedisKey := fmt.Sprintf("%s:%d", constants.WAREHOUSE_By_AGENT_ID, agentID)
	warehouseOnRedis, err := r.redisdb.Client().Get(ctx, warehouseRedisKey).Result()

	if err == redis.Nil {
		err = r.db.QueryRow("SELECT COUNT(*) as total FROM warehouses as w "+
			"INNER JOIN agents as a on a.id = w.owner_id "+
			"WHERE w.is_main = 1 AND a.id = ?", agentID).Scan(&total)
		...
		if countOnly == false {
			...
			err = r.db.QueryRow(""+
				...
				"INNER JOIN villages as v on v.id = w.village_id "+
				"INNER JOIN agents as a on a.id = w.owner_id "+
				"WHERE  w.is_main = 1 AND a.id = ?", agentID).
				Scan(&warehouse.ID, &warehouse.Code, &warehouse.Name, &warehouse.OwnerID, &warehouse.ProvinceID, &warehouse.ProvinceName, &warehouse.CityID, &warehouse.CityName, &warehouse.DistrictID, &warehouse.DistrictName, &warehouse.VillageID, &warehouse.VillageName, &warehouse.Address, &warehouse.Phone, &warehouse.MainMobilePhone, &warehouse.Email, &warehouse.PicName, &warehouse.Status, &warehouse.WarehouseTypeID, &warehouse.IsMain)

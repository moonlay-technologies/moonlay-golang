...
type SalesOrderLogRepositoryInterface interface {
	...
	Get(request *models.SalesOrderEventLogRequest, countOnly bool, ctx context.Context, result chan *models.SalesOrderLogsChan)
...
func (r *salesOrderLogRepository) Get(request *models.SalesOrderEventLogRequest, countOnly bool, ctx context.Context, resultChan chan *models.SalesOrderLogsChan) {
	response := &models.SalesOrderLogsChan{}
	...
	if request.SoCode != "" {
		filter = bson.M{"so_code": request.SoCode}
	}

	if request.Status != "" {
		filter = bson.M{"status": request.Status}
	}

	if request.Action != "" {
		filter = bson.M{"action": request.Action}
	}

	if request.AgentID > 0 {
		filter = bson.M{"data.agent_id": request.AgentID}
	}
	...
	if countOnly == false {
		salesOrderLogs := []*models.SalesOrderLog{}
		...
		for cursor.Next(ctx) {
			var salesOrderLog *models.SalesOrderLog
